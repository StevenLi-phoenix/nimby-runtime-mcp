...
s=json.loads(Path('data/nyc_west_end_bay_source.json').read_bytes());stop=json.loads(Path('data/nyc_d_plan.json').read_bytes())['directions']['to_coney_island']['stations'][32];centers=[]
for w in s['ways']:
 if w['osm_way_id'] in [427044486,427044487]:
...
p=dict(name='Bay Parkway',plan_index=32,center=c,scale=scale,southbound_axis=dict(x=tx,y=ty),tracks=tracks,source_platform_way_ids=[427044486,427044487],source='data/nyc_west_end_bay_source.json',layout='two_islands_three_physical_tracks',depth=2,stage='candidate_native_creation_pending');Path('data/nyc_west_end_bay_three_tracks.json').write_text(json.dumps(p,indent=2));print(json.dumps(rows))
