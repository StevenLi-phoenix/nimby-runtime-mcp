"""Plan Jamaica-179th Street's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_179_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][0]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [906708610,906708611]]
    ...
    return dict(name='Jamaica-179th Street', plan_index=0, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_179_source.json',
...
if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_179_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
