...
s=json.loads(Path('data/nyc_west_end_bay50_source.json').read_bytes());stop=json.loads(Path('data/nyc_d_plan.json').read_bytes())['directions']['to_coney_island']['stations'][34];centers=[]
for w in s['ways']:
 if w['osm_way_id'] in [427044480,427044482]:
  c=centroid([{k:p[k]-stop[k] for k in ['x','y']} for p in w['points']]);centers.append({k:c[k]+stop[k] for k in c})
c={k:sum(p[k] for p in centers)/2 for k in ['x','y']};ref=next(w for w in s['ways'] if w['osm_way_id']==702926823);_,_,(tx,ty)=nearest(ref['points'],c)
if ty>0:tx,ty=-tx,-ty
scale=math.cos(math.radians(stop['latitude']));rows=[]
for w in s['ways']:
 if w['tags'].get('railway')!='subway' or w['tags'].get('name')!='BMT West End Line' or w['tags'].get('service'):continue
...
p=dict(name='Bay 50th Street',plan_index=34,center=c,scale=scale,southbound_axis=dict(x=tx,y=ty),tracks=tracks,source_platform_way_ids=[427044480,427044482],source='data/nyc_west_end_bay50_source.json',layout='two_side_platforms_with_center_express_track',depth=2,stage='candidate_native_creation_pending');Path('data/nyc_west_end_bay50_three_tracks.json').write_text(json.dumps(p,indent=2));print(json.dumps(rows))
