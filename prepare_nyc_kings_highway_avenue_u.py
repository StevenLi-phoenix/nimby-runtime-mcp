"""Plan three source-connected elevated tracks from Kings Highway to Avenue U."""
import json
...
def prepare():
    ...
    source=json.loads(Path('data/nyc_lower_culver_source.json').read_bytes())
    nodes={n['id']:n for name in ['nyc-kings-highway-avenue-u-live'] for n in json.loads(Path('work/'+name+'.json').read_bytes())['nodes']}
    s={s['plan_index']:s for s in f['stations']}
    ends={'1':(s[39]['east_secondary'],s[40]['west_secondary']),
          '2':(s[39]['east_primary'],s[40]['west_primary']),
          '3-4':(f['kings_highway_center_platform']['south_endpoint'],f['avenue_u_center_track']['north_endpoint'])}
    scale=next(s for s in source['stations'] if s['plan_index']==40)['scale'];corridors=[]
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_lower_culver_source.json',
                stage='candidate_native_audit_pending',attribution='© OpenStreetMap contributors; https://www.openstreetmap.org/copyright')


if __name__=='__main__':
    result=prepare();Path('data/nyc_kings_highway_avenue_u_corridors.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
