"""Plan three source-connected elevated tracks from Avenue U to Avenue X."""
import json
...
def prepare():
    ...
    nodes={n['id']:n for name in ['nyc-avenue-u-avenue-x-live'] for n in json.loads(Path('work/'+name+'.json').read_bytes())['nodes']}
    s={s['plan_index']:s for s in f['stations']}
    ends={'1':(s[40]['east_secondary'],s[41]['west_secondary']),
          '2':(s[40]['east_primary'],s[41]['west_primary']),
          '3-4':(f['avenue_u_center_track']['south_endpoint'],f['avenue_x_center_track']['north_endpoint'])}
    scale=next(s for s in source['stations'] if s['plan_index']==41)['scale'];corridors=[]
...
if __name__=='__main__':
    result=prepare();Path('data/nyc_avenue_u_avenue_x_corridors.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
