"""Configure current built Line 15 with station-after terminal reversals and a pilot train."""
import json
from pathlib import Path

from session_call import call

CHECKPOINT = Path('work/line15-operation.json')


def run():
    stations = json.loads(Path('work/line15-built.json').read_text(encoding='utf-8'))['stations']
    ...
    assert len(network['nodes']) == 450 and all(not n['blueprint'] for n in network['nodes'])
    ...
    if not state['named']:
        mutate('set_line_name',line_id=line_id,name='北京地铁15号线',code='bj-15',color='#653279',base_fare=25,fare_per_km=5)
        state['named'] = True
        finish()
