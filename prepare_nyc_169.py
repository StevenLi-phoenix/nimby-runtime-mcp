"""Plan 169th Street Queens Boulevard side platforms."""
import json
import math
...
def prepare():
    source=json.loads(Path('data/nyc_169_source.json').read_bytes())
    stop=json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][1]
    footprints=[w for w in source['ways'] if w['osm_way_id'] in [906709556,906709557]]
    ...
    assert 3<spacing<30
    return dict(name='169th Street',plan_index=1,center=center,scale=scale,
                ...
                reserved_express_depth=-2,source='data/nyc_169_source.json',
                stage='candidate_native_creation_pending')


if __name__=='__main__':
    p=prepare();Path('data/nyc_169_side_platforms.json').write_text(json.dumps(p,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
