"""Create Changping Line station blueprints only; never commit construction implicitly."""
import hashlib
import json
import math
...
CHECKPOINT = Path('work/changping-built.json')


def run():
    raw = Path('data/changping_plan.json').read_bytes()
    plan = json.loads(raw)
