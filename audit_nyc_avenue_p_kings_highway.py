"""Audit the three elevated tracks from Avenue P to Kings Highway."""
import json
import math
import os
...
def audit():
    ...
    plan=json.loads(Path('data/nyc_avenue_p_kings_highway_corridors.json').read_bytes())
    before={n['id']:n for n in json.loads(Path('work/nyc-avenue-p-kings-highway-live.json').read_bytes())['nodes']}
    net=json.loads(Path('work/nyc-avenue-p-kings-highway-connected.json').read_bytes());nodes={n['id']:n for n in net['nodes']}
    corridors=f['avenue_p_kings_highway_corridors'];parents={i for c in corridors for i in [c['start_node_id'],c['end_node_id']]};changes=[]
    for i,n in before.items():
        fields=[k for k in n if n[k]!=nodes[i][k]]
        if fields:
            if i not in parents:
                assert i in f['kings_highway_center_platform']['nodes']
                assert {'previous','next','curve'}<=set(fields)<={'previous','next','curve','buildings'}
                assert len(n['buildings'])==len(nodes[i]['buildings'])
                for a,b in zip(n['buildings'],nodes[i]['buildings']):
                    changed={k for k in a if a[k]!=b[k]}
                    assert not changed or (a['type']==27 and changed=={'width'} and abs(a['width']-b['width'])<.00001)
            ...
            movement=math.hypot(n['x']-nodes[i]['x'],n['y']-nodes[i]['y'])*plan['scale']
            # Native paired endpoint follows the opposite approach curve; this is
            # a non-platform lead, with its original station/buildings unchanged.
            limit=.071 if i=='281475840147457' else .001
            assert movement<limit and (limit==.001 or n['station_id']=='0')
    ...
    ax.set(aspect='equal',ylim=(500,-50),title='Avenue P to Kings Highway',xlabel='Metres across',ylabel='Metres south');ax.grid(alpha=.2)
    fig.tight_layout();fig.savefig(Path(os.environ['TEMP'])/'nyc-avenue-p-kings-highway-audit.png',dpi=150)
    ...
    Path('work/nyc-avenue-p-kings-highway-audit.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
