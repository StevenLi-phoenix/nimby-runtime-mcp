"""Configure current built Changping Line with station-after terminal reversals and a pilot train."""
import json
from pathlib import Path

from session_call import call

CHECKPOINT = Path('work/changping-operation.json')


def run():
    stations = json.loads(Path('work/changping-built.json').read_text(encoding='utf-8'))['stations']
    ...
    built = json.loads(Path('work/changping-construction.json').read_text(encoding='utf-8'))['after']['nodes']
    assert {n['id'] for n in network['nodes']} == {n['id'] for n in built}
    assert all(not n['blueprint'] for n in network['nodes'])
    ...
    if not state['named']:
        mutate('set_line_name',line_id=line_id,name='北京地铁昌平线',code='bj-cp',color='#D47DAA',base_fare=25,fare_per_km=5)
        state['named'] = True
        finish()
    ...
    for index in range(line['stop_count'],len(stops)):
        ...
        finish()
        print(f'Stop {index+1}/40 configured',flush=True)
    ...
    save()
    print(json.dumps(dict(line_id=line_id,train_id=state['fleet'][0]['id'],stops=40)))
