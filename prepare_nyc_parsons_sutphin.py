"""Plan four through tracks between Parsons Boulevard and Sutphin Boulevard; yard links excluded."""
import json
...
def prepare():
    ...
    ns = {n['id']: n for n in json.loads(Path('work/nyc-parsons-sutphin-live.json').read_bytes())['nodes']}
    source_path = 'data/nyc_parsons_sutphin_source.json'
    source = json.loads(Path(source_path).read_bytes())
    scale = json.loads(Path('data/nyc_parsons_islands.json').read_bytes())['scale']
    local=s['sutphin_platform_groups'][0]
    express={e['track_ref']:e for e in s['sutphin_express_tracks']}
    pairs=[('1',s['parsons_islands'][0]['east_secondary'],local['west_secondary']),
           ('3',s['parsons_islands'][0]['east_primary'],express['3']['west']),
           ('4',s['parsons_islands'][1]['east_secondary'],express['4']['west']),
           ('2',s['parsons_islands'][1]['east_primary'],local['west_primary'])]
...
if __name__ == '__main__':
    p = prepare()
    Path('data/nyc_parsons_sutphin_corridors.json').write_text(json.dumps(p, indent=2)+'\n')
