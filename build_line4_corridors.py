"""Connect current Line 4 station blueprints; never build or restore old station geometry."""
import bisect
import json
import math
import sys
...
CHECKPOINT = Path('work/line4-corridors.json')


def run():
    stations = json.loads(Path('work/line4-built.json').read_text(encoding='utf-8'))['stations']
    plan = json.loads(Path('data/line4_plan.json').read_text(encoding='utf-8'))
    ...
    for i in range(len(state['corridors']), len(stations)-1):
        ...
        del state['active']
        save()
        print(f'Corridor {i+1}/23 connected', flush=True)
