"""Plan 75th Avenue Queens Boulevard side platforms."""
import json
import math
...
def prepare():
    ...
    stop=json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][6]
    footprints=[w for w in source['ways'] if w['osm_way_id'] in [906943204,906943205]]
    ...
    assert 3<spacing<30
    return dict(name='75th Avenue',plan_index=6,center=center,scale=scale,
                ...
                reserved_express_depth=-2,source='data/nyc_forest_hills_source.json',
                stage='candidate_native_creation_pending')


if __name__=='__main__':
    p=prepare();Path('data/nyc_seventy_fifth_side_platforms.json').write_text(json.dumps(p,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
