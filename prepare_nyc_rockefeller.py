"""Plan 47th-50th Streets-Rockefeller Center's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    ...
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][13]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [907921873, 907921874]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 46340246)
    ...
    return dict(name='47th-50th Streets-Rockefeller Center', plan_index=13, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_rockefeller_source.json',
...
if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_rockefeller_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
