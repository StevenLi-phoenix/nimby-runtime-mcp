"""Plan 59th Street-Columbus Circle's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_columbus_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_d_plan.json').read_bytes())['directions']['to_coney_island']['stations'][13]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [510806175, 510806176]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 46340129)
    ...
    return dict(name='59th Street-Columbus Circle', plan_index=13, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-3, platform_length_metres=140, source='data/nyc_columbus_source.json',
                stage='candidate_native_platform_audit_pending',
                pending=['Unify IND platforms under one native station.',
                         'Connect Sixth Avenue south to Bryant Park and retain the northern route splits.',
                         'Verify the northern junction layers against source geometry.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_columbus_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
