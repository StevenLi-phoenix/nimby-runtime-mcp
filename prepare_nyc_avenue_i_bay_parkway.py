"""Plan three source-connected elevated tracks from Avenue I to Bay Parkway."""
import json
...
def prepare():
    ...
    nodes={n['id']:n for name in ['nyc-avenue-i-bay-parkway-live'] for n in json.loads(Path('work/'+name+'.json').read_bytes())['nodes']}
    s={s['plan_index']:s for s in f['stations']}
    ends={'1':(s[35]['east_secondary'],s[36]['west_secondary']),
          '2':(s[35]['east_primary'],s[36]['west_primary']),
          '3-4':(f['avenue_i_center_track']['south_endpoint'],f['bay_parkway_center_track']['north_endpoint'])}
    scale=next(s for s in source['stations'] if s['plan_index']==36)['scale'];corridors=[]
...
if __name__=='__main__':
    result=prepare();Path('data/nyc_avenue_i_bay_parkway_corridors.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
