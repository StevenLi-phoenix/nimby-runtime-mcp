"""Audit the three elevated tracks from Avenue U to Avenue X."""
import json
import math
import os
...
def audit():
    ...
    plan=json.loads(Path('data/nyc_avenue_u_avenue_x_corridors.json').read_bytes())
    before={n['id']:n for n in json.loads(Path('work/nyc-avenue-u-avenue-x-live.json').read_bytes())['nodes']}
    net=json.loads(Path('work/nyc-avenue-u-avenue-x-connected.json').read_bytes());nodes={n['id']:n for n in net['nodes']}
    corridors=f['avenue_u_avenue_x_corridors'];parents={i for c in corridors for i in [c['start_node_id'],c['end_node_id']]};changes=[]
    for i,n in before.items():
        fields=[k for k in n if n[k]!=nodes[i][k]]
        if fields:
            if i=='281475842113538':
                assert fields==['curve']
                delta=LineString(n['curve']).hausdorff_distance(LineString(nodes[i]['curve']))*plan['scale']
                assert delta<.0001
                changes.append(dict(id=i,fields=fields,curve_difference_metres=delta))
                continue
            if i not in parents:
                assert i in f['avenue_x_center_track']['nodes']
    ...
    origin=before[groups['3-4'][0]];tx,ty=-.106,-.994;scale=plan['scale']
    def xy(x,y):
    ...
    ax.set(aspect='equal',ylim=(560,-50),title='Avenue U to Avenue X',xlabel='Metres across',ylabel='Metres south');ax.grid(alpha=.2)
    fig.tight_layout();fig.savefig(Path(os.environ['TEMP'])/'nyc-avenue-u-avenue-x-audit.png',dpi=150)
    ...
    Path('work/nyc-avenue-u-avenue-x-audit.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
