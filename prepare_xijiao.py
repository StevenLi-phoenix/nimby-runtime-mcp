"""Validate Xijiao Line geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '巴沟 颐和园西门 茶棚 万安 国家植物园 香山'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 8008876]
    ways = [objects['way', m['ref']] for m in relation['members']
            if m['type'] == 'way' and m['role'] == '']
    assert all(w.get('tags', {}).get('railway') == 'light_rail' for w in ways)
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Xijiao Line station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/8008876/full.json',
                attribution='© OpenStreetMap contributors, ODbL 1.0',
                source_sha256=hashlib.sha256(content).hexdigest(), relation_version=relation['version'],
                prepared_at=datetime.now(timezone.utc).isoformat(),
                projection='EPSG:3857; R=6378137', category='tram', track_type='Tram', source_railway='light_rail', simulation_note='按实际地面、桥梁和隧道保留线路几何；先完成 Medium 蓝图连接，再经 set_tram_track 转换；当前采购接口使用六节替代车辆模拟运营',
                scope='北京西郊线巴沟—香山，完整6站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-xijiao.html',
                code='bj-xj', color='#E1261C',
...
if __name__ == '__main__':
    ...
    Path('data/xijiao_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
