"""Validate Line 19 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '牡丹园 北太平庄 积水潭 平安里 太平桥 牛街 景风门 草桥 新发地 新宫'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 13625325]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 19 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/13625325/full.json',
                ...
                scope='北京地铁19号线牡丹园—新宫，当前完整10座运营站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-19.html',
                code='bj-19', color='#D6ABC1',
...
if __name__ == '__main__':
    ...
    Path('data/line19_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
