"""Plan York Street's two-track island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_york_street_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][23]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [904599540]]
    ...
    ref = min((w for w in source['ways'] if w['tags'].get('name') == 'IND Culver Line'), key=lambda w:nearest(w['points'],center)[0])
    ...
    for w in source['ways']:
        if w['tags'].get('railway') != 'subway' or w['tags'].get('level') != '-2':
            continue
        if w['tags'].get('name') not in ['IND Culver Line']:
            continue
    ...
    return dict(name='York Street', plan_index=23, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_york_street_source.json',
                reserved_other_services=[],
                stage='candidate_native_platform_audit_pending',
                pending=['Verify island platform buildings.',
                         'Connect F south to Jay Street and north to East Broadway.',
                         'Verify Rutgers Street Tunnel source geometry before crossing the river.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_york_street_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
