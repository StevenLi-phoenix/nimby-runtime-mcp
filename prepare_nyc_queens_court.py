"""Plan the Sixth Avenue connection from Queens Plaza to Court Square."""
import json
...
def prepare():
    ...
    source = json.loads(Path('data/nyc_court23_source.json').read_bytes())
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-queens-court-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_queens_plaza_islands.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('3',0,'east_secondary'),('4',1,'east_primary')]:
        a = state['queens_plaza_islands'][index]['east_primary' if ref=='3' else 'east_secondary']
        b = state['court23_platform_groups'][0][key.replace('east','west')]
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_court23_source.json',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_queens_court_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
