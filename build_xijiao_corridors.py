"""Connect current Xijiao Line station blueprints; never build or restore old station geometry."""
import bisect
import json
import math
import sys
...
CHECKPOINT = Path('work/xijiao-corridors.json')


def run():
    stations = json.loads(Path('work/xijiao-built.json').read_text(encoding='utf-8'))['stations']
    plan = json.loads(Path('data/xijiao_plan.json').read_text(encoding='utf-8'))
    ...
    for i in range(len(state['corridors']), len(stations)-1):
        ...
        if 'active' not in state:
            ...
            # Explicitly reviewed close source controls, see work/xijiao-short-control-review.json.
            # Preserve tunnel portal controls. Two bridges 22m apart share a short elevated connector;
            # retain the tiny second bridge and lower at the next control, 40m beyond its source end.
            omitted = {
                0: {5037948447, 8198474446, 8198474445, 4946947200, 5315236176, 9439699602},
                1: {4946947165, 4946947164},
            }.get(i, set())
            points = [p for p in points if p.get('osm_id') not in omitted]
        ...
        del state['active']
        save()
        print(f'Corridor {i+1}/5 connected', flush=True)
