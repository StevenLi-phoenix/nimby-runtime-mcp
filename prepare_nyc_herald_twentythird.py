"""Plan the Sixth Avenue connection from Herald Square to 23rd Street."""
import json
...
def prepare():
    ...
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-herald-twentythird-live.json').read_bytes())['nodes']}
    ...
    for ref, index, key in [('1', 0, 'east_secondary'), ('2', 1, 'east_primary')]:
        ...
        b = state['twentythird_platform_groups'][0]['west_secondary' if ref == '1' else 'west_primary']
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_twentythird_source.json',
                ...
                adaptation='Source -3 to -1 transition retained; original depths remain in source_layer_segments.',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_herald_twentythird_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
