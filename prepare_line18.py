"""Validate Line 18 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '马连洼 上地软件园 东北旺 龙泽西 回龙观西大街 文华路 回龙观东大街 霍营东 天通苑 太平庄 天通苑东'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 20010805]
    ...
    if len(ways) > 1 and ids[-1] not in (ways[1]['nodes'][0], ways[1]['nodes'][-1]):
        ids.reverse()
    for way in ways[1:]:
        part = list(way['nodes'])
        if part[-1] == ids[-1]:
    ...
    stops = [objects['node', m['ref']] for m in relation['members']
             if m['type'] == 'node' and objects['node', m['ref']].get('tags', {}).get('name') in EXPECTED]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 18 station order')
    ...
    stations = []
    for n in stops:
        source_x, source_y = project(n)
        if n['id'] in ids:
            node_index = ids.index(n['id'])
            s = chain[node_index]
            offset = 0.0
        else:
            # Five relation members are station centres rather than track stops.
            # Project only onto the validated route; reject offsets over 30 world m.
            candidates = []
            for j, (u, v) in enumerate(zip(route, route[1:])):
                ...
                if not length2:
                    continue
                f = max(0, min(1, ((source_x-u['x'])*dx+(source_y-u['y'])*dy)/length2))
                px, py = u['x']+f*dx, u['y']+f*dy
                candidates.append((math.hypot(px-source_x, py-source_y), j, chain[j]+f*math.sqrt(length2)))
            offset, node_index, s = min(candidates)
            assert offset < 30, f"Station centre too far from route: {n['id']}"
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/20010805/full.json',
                attribution='© OpenStreetMap contributors, ODbL 1.0',
                source_sha256=hashlib.sha256(content).hexdigest(), relation_version=relation['version'],
                prepared_at=datetime.now(timezone.utc).isoformat(),
                projection='EPSG:3857; R=6378137', category='metro', track_type='Medium speed', source_railway='subway',
                scope='北京地铁18号线马连洼—天通苑东，当前完整11座运营站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-18.html',
                code='bj-18', color='#5554A2',
...
if __name__ == '__main__':
    ...
    Path('data/line18_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
