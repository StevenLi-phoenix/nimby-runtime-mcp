"""Connect current Line 19 station blueprints; never build or restore old station geometry."""
import bisect
import json
import math
import sys
...
CHECKPOINT = Path('work/line19-corridors.json')


def run():
    stations = json.loads(Path('work/line19-built.json').read_text(encoding='utf-8'))['stations']
    plan = json.loads(Path('data/line19_plan.json').read_text(encoding='utf-8'))
    ...
    for i in range(len(state['corridors']), len(stations)-1):
        ...
        del state['active']
        save()
        print(f'Corridor {i+1}/9 connected', flush=True)
