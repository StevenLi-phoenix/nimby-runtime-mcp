"""Audit the Avenue I station and its three elevated approaches from 18 Avenue."""
import json
import math
import os
...
def audit():
    ...
    plan=json.loads(Path('data/nyc_eighteenth_avenue_i_corridors.json').read_bytes())
    before={n['id']:n for n in json.loads(Path('work/nyc-eighteenth-avenue-i-live.json').read_bytes())['nodes']}
    net=json.loads(Path('work/nyc-eighteenth-avenue-i-connected.json').read_bytes());nodes={n['id']:n for n in net['nodes']}
    corridors=f['eighteenth_avenue_i_corridors'];parents={c['start_node_id'] for c in corridors};changes=[]
    ...
    station=next(s for s in f['stations'] if s['plan_index']==35)
    added=set(station['nodes'])|set(f['avenue_i_center_track']['nodes'])|{i for c in corridors for i in c['nodes']}
    ...
    ax.set(aspect='equal',ylim=(300,-50),title='18 Avenue to Avenue I',xlabel='Metres across',ylabel='Metres south');ax.grid(alpha=.2)
    fig.tight_layout();fig.savefig(Path(os.environ['TEMP'])/'nyc-eighteenth-avenue-i-audit.png',dpi=150)
    ...
    Path('work/nyc-eighteenth-avenue-i-audit.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
