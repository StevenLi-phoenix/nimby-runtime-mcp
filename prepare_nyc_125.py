"""Plan 125th Street's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_125_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_d_plan.json').read_bytes())['directions']['to_coney_island']['stations'][12]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [685770066, 685770067]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 685546721)
    ...
    return dict(name='125th Street', plan_index=12, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_125_source.json',
                stage='candidate_native_platform_audit_pending',
                pending=['Unify IND platforms under one native station.',
                         'Connect south toward Columbus Circle with express and local tracks preserved.',
                         'Verify 135th Street and 145th Street approach tracks and junction layers.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_125_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
