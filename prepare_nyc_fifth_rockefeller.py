"""Plan the Sixth Avenue connection from Fifth Avenue/53rd Street to Rockefeller Center."""
import json
...
def prepare():
    ...
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-fifth-rockefeller-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_fifth_53rd_stacked.json').read_bytes())['scale']
    corridors = []
    for ref, index in [('1', 0), ('2', 1)]:
        a = state['fifth_53rd_platforms'][index]['west_lead']
        b = state['rockefeller_islands'][index]['west_primary']
        assert all('0' in [nodes[i]['previous'], nodes[i]['next']] for i in [a,b])
        eligible = []
        for w in source['ways']:
            name = w['tags'].get('name')
            original_ref = w['tags'].get('railway:track_ref')
            mapped = {'3':'1', '4':'2'}.get(original_ref) if name == 'IND Queens Boulevard Line' else original_ref
            if name not in ['IND Queens Boulevard Line','IND Sixth Avenue Line'] or mapped != ref:
                continue
            eligible.append(dict(w,tags=dict(w['tags'],name='IND Culver Line',**{'railway:track_ref':mapped})))
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_rockefeller_source.json',
                ...
                adaptation='Source junction layer transitions retained; original depths remain in source_layer_segments.',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_fifth_rockefeller_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
