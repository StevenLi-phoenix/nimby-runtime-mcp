"""Create Daxing station blueprints only; never commit construction implicitly."""
import hashlib
import json
import math
...
CHECKPOINT = Path('work/daxing-built.json')


def run():
    raw = Path('data/daxing_plan.json').read_bytes()
    plan = json.loads(raw)
    ...
    for index, s in enumerate([s for s in plan['stations'] if not s['reuse_existing']]):
        ...
        if record['stage'] == 'assigned':
            ...
            save()
        print(f"Station {index+1}/11 verified", flush=True)
