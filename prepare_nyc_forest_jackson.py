"""Plan the Sixth Avenue connection from Forest Hills to Jackson Heights."""
import json
...
def prepare():
    ...
    source = json.loads(Path('data/nyc_forest_jackson_source.json').read_bytes())
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-forest-jackson-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_forest_hills_islands.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('3',0,'east_secondary'),('4',1,'east_primary')]:
        a = state['forest_hills_islands'][index]['east_primary' if ref=='3' else 'east_secondary']
        b = state['jackson_islands'][index]['west_primary' if ref=='3' else 'west_secondary']
        ...
        # Limit native tangent drift into the future local tracks on long
        # station approaches. Keep these controls on the source geometry.
        for d in [120, chain[-1]-320, chain[-1]-160]:
            ...
            points.append(dict(x=route[i]['x']+f*(route[i+1]['x']-route[i]['x']),
                               ...
                               depth=depths[i],metres=d,reason='station_approach_spacing'))
        points.sort(key=lambda p: p['metres'])
        if len(points) == 2:
            interior=[]
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_forest_jackson_source.json',
                stage='candidate_native_curves_and_crossing_audit_pending',
                adaptation='Source junction layers retained; original depths remain in source_layer_segments.',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_forest_jackson_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
