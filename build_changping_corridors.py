"""Connect current Changping Line station blueprints; never build or restore old station geometry."""
import bisect
import json
import math
import sys
...
CHECKPOINT = Path('work/changping-corridors.json')


def run():
    stations = json.loads(Path('work/changping-built.json').read_text(encoding='utf-8'))['stations']
    plan = json.loads(Path('data/changping_plan.json').read_text(encoding='utf-8'))
    ...
    for i in range(len(state['corridors']), len(stations)-1):
        ...
        if 'active' not in state:
            ...
            # Xi'erqi-Qinghe: redundant collinear viaduct point 11m from its neighbour.
            # Keep the actual Ground/tunnel boundary nodes and all station endpoints.
            if i == 11:
                points = [p for p in points if p.get('osm_id') != 9480765289]
        ...
        del state['active']
        save()
        print(f'Corridor {i+1}/19 connected', flush=True)
