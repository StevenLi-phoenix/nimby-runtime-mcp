"""Plan four through tracks between Kew Gardens and 75 Avenue; yard links excluded."""
import json
...
def prepare():
    ...
    source_path = 'data/nyc_kew_75_source.json'
    ...
    pairs = [('1', s['kew_gardens_islands'][0]['east_secondary'], local['west_secondary']),
             ('3', s['kew_gardens_islands'][0]['east_primary'], express['3']['west']),
             ('4', s['kew_gardens_islands'][1]['east_secondary'], express['4']['west']),
             ('2', s['kew_gardens_islands'][1]['east_primary'], local['west_primary'])]
    ...
    return dict(source=source_path, scale=scale, corridors=corridors,
                ...
                pending=['Audit native curves, station approaches and source junctions.',
                         'Keep yard tracks and yard access unbuilt.'])


if __name__ == '__main__':
    p = prepare()
    Path('data/nyc_kew_75_corridors.json').write_text(json.dumps(p, indent=2)+'\n')
