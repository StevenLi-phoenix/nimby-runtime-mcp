"""Plan three source-connected elevated tracks from Avenue N to Avenue P."""
import json
...
def prepare():
    ...
    nodes={n['id']:n for name in ['nyc-avenue-n-avenue-p-live'] for n in json.loads(Path('work/'+name+'.json').read_bytes())['nodes']}
    s={s['plan_index']:s for s in f['stations']}
    ends={'1':(s[37]['east_secondary'],s[38]['west_secondary']),
          '2':(s[37]['east_primary'],s[38]['west_primary']),
          '3-4':(f['avenue_n_center_track']['south_endpoint'],f['avenue_p_center_track']['north_endpoint'])}
    scale=next(s for s in source['stations'] if s['plan_index']==38)['scale'];corridors=[]
...
if __name__=='__main__':
    result=prepare();Path('data/nyc_avenue_n_avenue_p_corridors.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
