"""Validate Line 9 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '郭公庄 丰台科技园 科怡路 丰台南路 丰台东大街 七里庄 六里桥 六里桥东 北京西站 军事博物馆 白堆子 白石桥南 国家图书馆'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 2063278]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 9 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/2063278/full.json',
                ...
                scope='北京地铁9号线郭公庄—国家图书馆，完整13站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.bjsubway.com/station/xltcx/line9/2013-08-23/158.html',
                code='bj-9', color='#87D300',
...
if __name__ == '__main__':
    ...
    Path('data/line9_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
