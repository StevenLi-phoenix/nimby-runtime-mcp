"""Audit the three elevated tracks from Bay Parkway to Avenue N."""
import json
import math
import os
...
def audit():
    ...
    plan=json.loads(Path('data/nyc_bay_parkway_avenue_n_corridors.json').read_bytes())
    before={n['id']:n for n in json.loads(Path('work/nyc-bay-parkway-avenue-n-live.json').read_bytes())['nodes']}
    net=json.loads(Path('work/nyc-bay-parkway-avenue-n-connected.json').read_bytes());nodes={n['id']:n for n in net['nodes']}
    corridors=f['bay_parkway_avenue_n_corridors'];parents={i for c in corridors for i in [c['start_node_id'],c['end_node_id']]};changes=[]
    for i,n in before.items():
        fields=[k for k in n if n[k]!=nodes[i][k]]
        if fields:
            if i not in parents:
                assert i in f['avenue_n_center_track']['nodes']
    ...
    ax.set(aspect='equal',ylim=(450,-50),title='Bay Parkway to Avenue N',xlabel='Metres across',ylabel='Metres south');ax.grid(alpha=.2)
    fig.tight_layout();fig.savefig(Path(os.environ['TEMP'])/'nyc-bay-parkway-avenue-n-audit.png',dpi=150)
    ...
    Path('work/nyc-bay-parkway-avenue-n-audit.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
