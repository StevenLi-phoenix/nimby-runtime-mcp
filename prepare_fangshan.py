"""Validate Fangshan Line geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '阎村东 苏庄 良乡南关 良乡大学城西 良乡大学城 良乡大学城北 广阳城 篱笆房 长阳 稻田 大葆台 郭公庄 白盆窑 花乡东桥 首经贸 东管头南'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 1721085]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Fangshan Line station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/1721085/full.json',
                ...
                scope='北京地铁房山线阎村东—东管头南，完整16站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.bjsubway.com/station/xltcx/fangshan/2013-10-26/358.html',
                code='bj-fs', color='#D85F26',
...
if __name__ == '__main__':
    ...
    Path('data/fangshan_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
