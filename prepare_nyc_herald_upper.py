...
def prepare():
    ...
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [511809787, 511809790]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 817381417)
    ...
    for w in source['ways']:
        if w['tags'].get('railway') != 'subway' or w['tags'].get('level') != '-2':
            continue
        if w['tags'].get('name') not in ['BMT Broadway Line']:
            continue
    ...
    return dict(name='34th Street-Herald Square', plan_index=15, center=center, scale=scale,
                ...
                depth=-2, platform_length_metres=140, source='data/nyc_twentythird_source.json',
                lower_sixth_platforms=dict(source_way_ids=[511809778,511809779],depth=-3),
                stage='candidate_native_platform_audit_pending',
                pending=['Unify IND platforms under one native station.',
                         'Connect Broadway tracks south to 28th Street and north to Times Square.',
                         'Unify upper platforms with the built Sixth Avenue station; preserve PATH source facilities.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_herald_upper_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
