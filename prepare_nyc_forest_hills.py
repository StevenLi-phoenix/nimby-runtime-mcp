"""Plan Forest Hills-71st Avenue's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_forest_hills_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][7]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [906943663,906943664]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 818901156)
    ...
    for w in source['ways']:
        if w['tags'].get('railway') != 'subway' or w['tags'].get('level') != '-2':
            continue
        if w['tags'].get('name') not in ['IND Queens Boulevard Line']:
            continue
    ...
    return dict(name='Forest Hills-71st Avenue', plan_index=7, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_forest_hills_source.json',
                stage='candidate_native_platform_audit_pending',
                pending=['Unify IND platforms under one native station.',
                         'Connect Queens Boulevard express and local tracks in both directions.',
                         'Retain the local terminal reversal requirement; defer yard access.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_forest_hills_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
