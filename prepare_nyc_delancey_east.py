"""Plan the Sixth Avenue connection from Delancey to East Broadway."""
import json
...
def prepare():
    ...
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-delancey-east-live.json').read_bytes())['nodes']}
    ...
    for ref, index, key in [('1', 0, 'east_secondary'), ('2', 1, 'east_primary')]:
        a = state['delancey_platform_groups'][0][key]
        b = state['east_broadway_islands'][0]['west_secondary' if ref == '1' else 'west_primary']
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_delancey_east_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
