"""Plan 23rd Street Sixth Avenue side platforms."""
import json
import math
...
def prepare():
    ...
    stop=json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][16]
    footprints=[w for w in source['ways'] if w['osm_way_id'] in [579071551,579071552]]
    ...
    rails=[w for w in source['ways'] if w['tags'].get('name')=='IND Sixth Avenue Line' and w['tags'].get('level')=='-1']
    ...
    for ref in ['1','2']:
        ...
        tracks.append(dict(track_ref=ref,**q,points=[dict(x=q['x']+d*tx/scale,y=q['y']+d*ty/scale,depth=-1) for d in [-120,-70,70,120]]))
    spacing=math.hypot(tracks[0]['x']-tracks[1]['x'],tracks[0]['y']-tracks[1]['y'])*scale
    assert 3<spacing<30
    return dict(name='23rd Street | Sixth Avenue',plan_index=16,center=center,scale=scale,
                southbound_axis=dict(x=tx,y=ty),tracks=tracks,track_spacing_metres=spacing,
                platform_type='two side platforms',platform_length_metres=140,depth=-1,
                source_platforms=footprints,reserved_path_platforms=[578686028,578686029],
                reserved_path_depth=-1,stage='candidate_native_creation_pending')


if __name__=='__main__':
    p=prepare();Path('data/nyc_twentythird_side_platforms.json').write_text(json.dumps(p,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
