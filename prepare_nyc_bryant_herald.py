"""Plan the Sixth Avenue connection from Bryant Park to Herald Square."""
import json
...
def prepare():
    ...
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-bryant-herald-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_bryant_islands.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('1', 0, 'east_secondary'), ('3', 0, 'east_primary'), ('4', 1, 'east_secondary'), ('2', 1, 'east_primary')]:
        a = state['bryant_islands'][index][key]
        b = state['herald_islands'][index][key.replace('east','west')]
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_bryant_source.json',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_bryant_herald_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
