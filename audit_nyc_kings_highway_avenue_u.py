"""Audit the three elevated tracks from Kings Highway to Avenue U."""
import json
import math
import os
...
def audit():
    ...
    plan=json.loads(Path('data/nyc_kings_highway_avenue_u_corridors.json').read_bytes())
    before={n['id']:n for n in json.loads(Path('work/nyc-kings-highway-avenue-u-live.json').read_bytes())['nodes']}
    net=json.loads(Path('work/nyc-kings-highway-avenue-u-connected.json').read_bytes());nodes={n['id']:n for n in net['nodes']}
    corridors=f['kings_highway_avenue_u_corridors'];parents={i for c in corridors for i in [c['start_node_id'],c['end_node_id']]};changes=[]
    for i,n in before.items():
        fields=[k for k in n if n[k]!=nodes[i][k]]
        if fields:
            if i not in parents:
                assert i in f['avenue_u_center_track']['nodes']
    ...
    ax.set(aspect='equal',ylim=(610,-50),title='Kings Highway to Avenue U',xlabel='Metres across',ylabel='Metres south');ax.grid(alpha=.2)
    fig.tight_layout();fig.savefig(Path(os.environ['TEMP'])/'nyc-kings-highway-avenue-u-audit.png',dpi=150)
    ...
    Path('work/nyc-kings-highway-avenue-u-audit.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
