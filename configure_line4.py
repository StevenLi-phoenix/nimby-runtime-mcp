"""Configure current built Line 4 with station-after terminal reversals and a pilot train."""
import json
from pathlib import Path

from session_call import call

CHECKPOINT = Path('work/line4-operation.json')
...
def run():
    ...
    assert len(network['nodes']) == 466 and all(not n['blueprint'] for n in network['nodes'])
    ...
    if not state['named']:
        mutate('set_line_name',line_id=line_id,name='北京地铁4号线',code='bj-4',color='#008E9C',base_fare=25,fare_per_km=5)
        state['named'] = True
        finish()
    ...
    for index in range(line['stop_count'],len(stops)):
        ...
        finish()
        print(f'Stop {index+1}/48 configured',flush=True)
    ...
    save()
    print(json.dumps(dict(line_id=line_id,train_id=state['fleet'][0]['id'],stops=48)))
