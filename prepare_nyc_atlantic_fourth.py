"""Plan Atlantic Avenue-Barclays Center's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_atlantic_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_d_plan.json').read_bytes())['directions']['to_coney_island']['stations'][21]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [510502279,510502288]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 817936186)
    ...
    return dict(name='Atlantic Avenue-Barclays Center', plan_index=21, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-3, platform_length_metres=140, source='data/nyc_atlantic_source.json',
...
if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_atlantic_fourth_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
