"""Connect current Line 16 station blueprints; never build or restore old station geometry."""
import bisect
import json
import math
import sys
...
CHECKPOINT = Path('work/line16-corridors.json')


def run():
    stations = json.loads(Path('work/line16-built.json').read_text(encoding='utf-8'))['stations']
    plan = json.loads(Path('data/line16_plan.json').read_text(encoding='utf-8'))
    ...
    for i in range(len(state['corridors']), len(stations)-1):
        ...
        if 'active' not in state:
            ...
            if i == 18:
                # Two same-depth controls are only 14m apart. Keep the later
                # point adjacent to the grade transition and both live endpoints.
                points = [p for p in points if p.get('osm_id') != 12438168819]
        ...
        del state['active']
        save()
        print(f'Corridor {i+1}/29 connected', flush=True)
