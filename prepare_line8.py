"""Validate Line 8 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '瀛海 德茂 五福堂 火箭万源 东高地 和义 大红门南 大红门 海户屯 木樨园 永定门外 天桥 珠市口 前门 王府井 金鱼胡同 中国美术馆 南锣鼓巷 什刹海 鼓楼大街 安德里北街 安华桥 北土城 奥体中心 奥林匹克公园 森林公园南门 林萃桥 永泰庄 西小口 育新 霍营 回龙观东大街 平西府 育知路 朱辛庄'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 1721067]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 8 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/1721067/full.json',
                ...
                scope='北京地铁8号线瀛海—朱辛庄，35站；完整线路规划',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.bjsubway.com/station/xltcx/line8/2018-12-30/546.html',
                code='bj-8', color='#00997A',
...
if __name__ == '__main__':
    ...
    Path('data/line8_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
