...
s=json.loads(Path('data/nyc_west_end_25_source.json').read_bytes());stop=json.loads(Path('data/nyc_d_plan.json').read_bytes())['directions']['to_coney_island']['stations'][33];centers=[]
for w in s['ways']:
 if w['osm_way_id'] in [427044435,427044436]:
  c=centroid([{k:p[k]-stop[k] for k in ['x','y']} for p in w['points']]);centers.append({k:c[k]+stop[k] for k in c})
c={k:sum(p[k] for p in centers)/2 for k in ['x','y']};ref=next(w for w in s['ways'] if w['osm_way_id']==813334681);_,_,(tx,ty)=nearest(ref['points'],c)
if ty>0:tx,ty=-tx,-ty
...
p=dict(name='25th Avenue',plan_index=33,center=c,scale=scale,southbound_axis=dict(x=tx,y=ty),tracks=tracks,source_platform_way_ids=[427044435,427044436],source='data/nyc_west_end_25_source.json',layout='two_side_platforms_with_center_express_track',depth=2,stage='candidate_native_creation_pending');Path('data/nyc_west_end_25_three_tracks.json').write_text(json.dumps(p,indent=2));print(json.dumps(rows))
