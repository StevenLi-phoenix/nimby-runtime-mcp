"""Configure current built Line 7 with station-after terminal reversals and a pilot train."""
import json
from pathlib import Path

from session_call import call

CHECKPOINT = Path('work/line7-operation.json')


def run():
    stations = json.loads(Path('work/line7-built.json').read_text(encoding='utf-8'))['stations']
    ...
    built = json.loads(Path('work/line7-construction.json').read_text(encoding='utf-8'))['after']['nodes']
    ...
    if not state['named']:
        mutate('set_line_name',line_id=line_id,name='北京地铁7号线',code='bj-7',color='#FFC66E',base_fare=25,fare_per_km=5)
        state['named'] = True
        finish()
