"""Checkpoint each native MCP command for the complete Beijing Line 2 ring.
...
CHECKPOINT = Path('work/line2-built.json')


def run():
    plan = json.loads(Path('data/line2_plan.json').read_text(encoding='utf-8'))
    ...
    if 'initial_runtime' not in state:
        ...
        save()
    assert status['cash'] > 500_000_000, 'Recheck construction budget'
    ...
    for s in plan['stations'][len(state['stations']):]:
        ...
        print(f"Station {len(state['stations'])}/18: {s['name']}", flush=True)
    ...
    for i in range(len(state['corridors']), 18):
        first, second = stations[i], stations[(i+1) % 18]
        if 'active_corridor' not in state:
            ...
            end_chain = second['chainage'] if i < 17 else chain[-1]
            points = simplify([start]+[p for p, c in zip(route, chain) if first['chainage']+margin < c < end_chain-margin]+[end])
        ...
        save()
        print(f"Corridor {i+1}/18: {first['name']} — {second['name']}", flush=True)
