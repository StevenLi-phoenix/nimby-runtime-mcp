"""Plan four through tracks between 169th Street and Parsons Boulevard; yard links excluded."""
import json
...
def prepare():
    ...
    ns = {n['id']: n for n in json.loads(Path('work/nyc-169-parsons-live.json').read_bytes())['nodes']}
    source_path = 'data/nyc_169_parsons_source.json'
    source = json.loads(Path(source_path).read_bytes())
    scale = json.loads(Path('data/nyc_169_side_platforms.json').read_bytes())['scale']
    local = s['169_platform_groups'][0]
    express = {e['track_ref']: e for e in s['169_express_tracks']}
    pairs = [('1', local['east_secondary'], s['parsons_islands'][0]['west_secondary']),
             ('3', express['3']['east'], s['parsons_islands'][0]['west_primary']),
             ('4', express['4']['east'], s['parsons_islands'][1]['west_secondary']),
             ('2', local['east_primary'], s['parsons_islands'][1]['west_primary'])]
...
if __name__ == '__main__':
    p = prepare()
    Path('data/nyc_169_parsons_corridors.json').write_text(json.dumps(p, indent=2)+'\n')
