"""Plan the Sixth Avenue connection from Jackson Heights to Queens Plaza."""
import json
...
def prepare():
    ...
    source = json.loads(Path('data/nyc_queens_express_source.json').read_bytes())
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-queens-express-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_jackson_islands.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('3',0,'east_secondary'),('4',1,'east_primary')]:
        a = state['jackson_islands'][index]['east_primary' if ref=='3' else 'east_secondary']
        b = state['queens_plaza_islands'][index]['west_primary' if ref=='3' else 'west_secondary']
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_queens_express_source.json',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_queens_express_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
