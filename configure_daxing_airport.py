"""Configure current built Daxing Airport Line with station-after terminal reversals and a pilot train."""
import json
from pathlib import Path

from session_call import call

CHECKPOINT = Path('work/daxing_airport-operation.json')
...
def run():
    ...
    built = json.loads(Path('work/daxing_airport-construction.json').read_text(encoding='utf-8'))['after']['nodes']
    ...
    if not state['named']:
        mutate('set_line_name',line_id=line_id,name='北京大兴机场线',code='bj-dae',color='#3F48CC',base_fare=25,fare_per_km=5)
        state['named'] = True
        finish()
    ...
    for index in range(line['stop_count'],len(stops)):
        ...
        finish()
        print(f'Stop {index+1}/6 configured',flush=True)
    ...
    save()
    print(json.dumps(dict(line_id=line_id,train_id=state['fleet'][0]['id'],stops=6)))
