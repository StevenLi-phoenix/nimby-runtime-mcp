"""Validate Line S1 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '石厂 小园 栗园庄 上岸 桥户营 四道桥 金安桥 苹果园'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 8008812]
    ways = [objects['way', m['ref']] for m in relation['members']
            if m['type'] == 'way' and m['role'] == '']
    assert all(w.get('tags', {}).get('railway') == 'monorail' for w in ways)
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line S1 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/8008812/full.json',
                attribution='© OpenStreetMap contributors, ODbL 1.0',
                source_sha256=hashlib.sha256(content).hexdigest(), relation_version=relation['version'],
                prepared_at=datetime.now(timezone.utc).isoformat(),
                projection='EPSG:3857; R=6378137', category='maglev', track_type='Medium speed', source_railway='monorail', simulation_note='当前接口使用普通轨道及替代列车模拟运营；不声称磁悬浮物理制式' ,
                scope='北京地铁S1线石厂—苹果园，完整8站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-s1.html',
                code='bj-s1', color='#A65A2A',
...
if __name__ == '__main__':
    ...
    Path('data/s1_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
