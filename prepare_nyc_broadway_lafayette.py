"""Plan Broadway-Lafayette Street's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_broadway_lafayette_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][19]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [907565476, 907565477]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 818168007)
    ...
    return dict(name='Broadway-Lafayette Street', plan_index=19, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-3, platform_length_metres=140, source='data/nyc_broadway_lafayette_source.json',
                reserved_bleecker_platforms=dict(source_way_ids=[907565474,907565475],depth=-1),
                stage='candidate_native_platform_audit_pending',
                pending=['Unify IND platforms under one native station.',
                         'Connect F south to 2nd Avenue and north to West 4th Street.',
                         'Preserve B/D tracks and Bleecker Street interchange for full network construction.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_broadway_lafayette_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
