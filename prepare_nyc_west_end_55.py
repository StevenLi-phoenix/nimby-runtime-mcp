...
s=json.loads(Path('data/nyc_west_end_55_source.json').read_bytes());stop=json.loads(Path('data/nyc_d_plan.json').read_bytes())['directions']['to_coney_island']['stations'][26];centers=[]
for w in s['ways']:
 if w['osm_way_id'] in [427044439,427044440]:
...
p=dict(name='55th Street',plan_index=26,center=c,scale=scale,southbound_axis=dict(x=tx,y=ty),tracks=tracks,source_platform_way_ids=[427044439,427044440],source='data/nyc_west_end_55_source.json',layout='two_side_platforms_with_center_express_track',depth=2,stage='candidate_native_creation_pending');Path('data/nyc_west_end_55_three_tracks.json').write_text(json.dumps(p,indent=2));print(json.dumps(rows))
