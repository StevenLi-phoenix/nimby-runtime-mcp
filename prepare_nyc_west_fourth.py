...
def prepare():
    ...
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [552495904, 552495905]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 554387912)
    ...
    return dict(name='West 4th Street-Washington Square', plan_index=18, center=center, scale=scale,
                ...
                depth=-3, platform_length_metres=140, source='data/nyc_west_fourth_source.json',
                reserved_eighth_avenue_platforms=dict(source_way_ids=[552742165,552742166],depth=-2),
                source_depth=-4, adaptation="Sixth Avenue source -4 maps to native -3; Eighth Avenue remains -2",
                stage='candidate_native_platform_audit_pending',
                pending=['Unify IND platforms under one native station.',
                         'Connect F south to Broadway-Lafayette and north to 14th Street.',
                         'Build upper Eighth Avenue islands and preserve all station junctions for full network construction.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_west_fourth_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
