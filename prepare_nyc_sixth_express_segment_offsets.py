...
def evaluate():
 ...
 for c in p['corridors']:
  ...
  for shift in [0.7,0.8,0.9,1.0,1.1,1.2,1.3,1.4]:
   minima={'up_PATH':999,'up_IND Sixth Avenue Line':999,'down_PATH':999,'down_IND Sixth Avenue Line':999};worst={}
   for d in range(1180,1701,2):
    ...
    for w,rail in ways:
     ...
     gap=q.distance(rail)*scale;name=('up_' if d<1430 else 'down_')+w['tags']['name']
...
if __name__=='__main__':
 r=evaluate();Path('data/nyc_sixth_express_segment_comparison.json').write_text(json.dumps(r,indent=2),encoding='utf-8');print([(q['ref'],q['offset_metres'],{k:round(v,3) for k,v in q['minima'].items()}) for q in r])
