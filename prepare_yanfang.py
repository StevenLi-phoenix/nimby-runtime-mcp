"""Validate Yanfang Line geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '阎村东 紫草坞 阎村 星城 大石河东 马各庄 饶乐府 房山城关 燕山'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 8469053]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Yanfang Line station order')
        ...
        for station in stations:
            ...
            if current['matches']:
                ...
                if station['depth'] in occupied:
                    if station['name'] == '阎村东' and station['depth'] == 1 and occupied == {1}:
                        # Source Yanfang platforms run parallel north of the live Fangshan platforms.
                        # Preserve both elevated layouts; audit actual approach/tail crossings after creation.
                        a, b = station['start'], station['end']
                        dx, dy = b['x']-a['x'], b['y']-a['y']
                        length = math.hypot(dx, dy)
                        distances = [abs(dx*(n['y']-a['y'])-dy*(n['x']-a['x']))/length
                                     for n in current['nearby']['nodes'] if n['station_id'] == match['id']]
                        assert min(distances) > 15, 'Source platforms too close: individual layout required'
                        station['shared_elevated_layout'] = dict(min_existing_axis_distance_world=min(distances),
                            rationale='OSM燕房站台位于既有房山站台北侧；保留平行高架，按轨道归属合站，区间及折返另审计')
                    else:
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/8469053/full.json',
                ...
                scope='北京地铁燕房线阎村东—燕山，完整9站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-yanfang.html',
                code='bj-yf', color='#D85F26',
...
if __name__ == '__main__':
    ...
    Path('data/yanfang_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
