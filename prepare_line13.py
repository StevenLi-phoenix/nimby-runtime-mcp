"""Validate Line 13 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
def prepare(path):
    ...
    relation = objects['relation', 1667375]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 13 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/1667375/full.json',
                ...
                scope='北京地铁13号线西直门—东直门，17站；完整线路规划',
                first_opening=dict(start='西直门', end='霍营', exclude_later_stations=['清河']),
                closed=False, world_length=chain[-1], stations=stations, route=route,
                source_segments=segments,
                elevation_policy='按实际区段采用OSM bridge/tunnel/layer；清河高架由用户确认',
...
if __name__ == '__main__':
    ...
    Path('data/line13_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
