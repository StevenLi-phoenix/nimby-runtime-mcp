"""Plan 42nd Street-Bryant Park's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_bryant_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][14]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [907922927, 907922928]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 819186607)
    ...
    return dict(name='42nd Street-Bryant Park', plan_index=14, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_bryant_source.json',
                reserved_flushing_platform=dict(source_way_id=903249946,source_depth=-4),
                stage='candidate_native_platform_audit_pending',
                pending=['Unify IND platforms under one native station.',
                         'Connect Sixth Avenue south to Herald Square and north to Rockefeller Center.',
                         'Preserve Flushing interchange and verify native depth adaptation.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_bryant_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
