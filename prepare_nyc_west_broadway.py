"""Plan the Sixth Avenue connection from West 4th Street to Broadway-Lafayette Street."""
import json
...
def prepare():
    ...
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-west-broadway-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_west_fourth_islands.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('1', 0, 'east_secondary'), ('3', 0, 'east_primary'), ('4', 1, 'east_secondary'), ('2', 1, 'east_primary')]:
        a = state['west_fourth_islands'][index][key]
        b = state['broadway_lafayette_islands'][index][key.replace('east','west')]
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_west_fourth_source.json',
                stage='candidate_native_curves_and_crossing_audit_pending',
                adaptation='Source -4 maps to native -3; original depths remain in source_layer_segments.',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_west_broadway_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
