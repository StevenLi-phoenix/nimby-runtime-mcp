"""Plan three source-connected elevated tracks from 18 Avenue to Avenue I."""
import json
...
def prepare():
    ...
    nodes={n['id']:n for name in ['nyc-eighteenth-avenue-i-live','nyc-avenue-i-native'] for n in json.loads(Path('work/'+name+'.json').read_bytes())['nodes']}
    s={s['plan_index']:s for s in f['stations']}
    ends={'1':(s[34]['east_secondary'],s[35]['west_secondary']),
          '2':(s[34]['east_primary'],s[35]['west_primary']),
          '3-4':(f['eighteenth_center_platform']['south_endpoint'],f['avenue_i_center_track']['north_endpoint'])}
    scale=next(s for s in source['stations'] if s['plan_index']==35)['scale'];corridors=[]
...
if __name__=='__main__':
    result=prepare();Path('data/nyc_eighteenth_avenue_i_corridors.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
