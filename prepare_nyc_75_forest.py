"""Plan four through tracks between 75 Avenue and Forest Hills; yard links excluded."""
import json
...
def prepare():
    ...
    ns = {n['id']: n for n in json.loads(Path('work/nyc-75-forest-live.json').read_bytes())['nodes']}
    source_path = 'data/nyc_forest_hills_source.json'
    source = json.loads(Path(source_path).read_bytes())
    scale = json.loads(Path('data/nyc_seventy_fifth_side_platforms.json').read_bytes())['scale']
    local = s['seventy_fifth_platform_groups'][0]
    express = {e['track_ref']: e for e in s['seventy_fifth_express_tracks']}
    pairs = [('1', local['east_secondary'], s['forest_hills_islands'][0]['west_secondary']),
             ('3', express['3']['east'], s['forest_hills_islands'][0]['west_primary']),
             ('4', express['4']['east'], s['forest_hills_islands'][1]['west_secondary']),
             ('2', local['east_primary'], s['forest_hills_islands'][1]['west_primary'])]
    ...
    return dict(source=source_path, scale=scale, corridors=corridors,
                ...
                pending=['Audit native curves and attach express/local crossovers for F service.',
                         'Keep yard tracks and yard access unbuilt.'])


if __name__ == '__main__':
    p = prepare()
    Path('data/nyc_75_forest_corridors.json').write_text(json.dumps(p, indent=2)+'\n')
