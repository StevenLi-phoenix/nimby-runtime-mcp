"""Validate Line 11 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '模式口 金安桥 北辛安 新首钢'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 13623625]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 11 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/13623625/full.json',
                ...
                scope='北京地铁11号线模式口—新首钢，当前完整4座运营站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-11.html',
                code='bj-11', color='#ED796A',
...
if __name__ == '__main__':
    ...
    Path('data/line11_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
