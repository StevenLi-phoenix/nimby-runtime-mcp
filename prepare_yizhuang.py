"""Validate Yizhuang Line geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '宋家庄 肖村 小红门 旧宫 亦庄桥 亦庄文化园 万源街 荣京东街 荣昌东街 同济南路 经海路 次渠南 次渠 亦庄火车站'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 2201486]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Yizhuang Line station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/2201486/full.json',
                ...
                scope='北京地铁亦庄线宋家庄—亦庄火车站，完整14站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.bjsubway.com/station/xltcx/lineyz/2013-10-26/359.html',
                code='bj-yz', color='#D61572',
...
if __name__ == '__main__':
    ...
    Path('data/yizhuang_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
