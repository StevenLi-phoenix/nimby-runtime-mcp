"""Validate Line 3 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '东四十条 工人体育场 团结湖 朝阳公园 石佛营 朝阳站 姚家园 东坝南 东坝 东坝北'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 18420550]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 3 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/18420550/full.json',
                ...
                scope='北京地铁3号线东四十条—东坝北，当前完整10座运营站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-3.html',
                code='bj-3', color='#C8102E',
...
if __name__ == '__main__':
    ...
    Path('data/line3_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
