"""Connect current Line 11 station blueprints; never build or restore old station geometry."""
import bisect
import json
import math
import sys
...
CHECKPOINT = Path('work/line11-corridors.json')


def run():
    stations = json.loads(Path('work/line11-built.json').read_text(encoding='utf-8'))['stations']
    plan = json.loads(Path('data/line11_plan.json').read_text(encoding='utf-8'))
    ...
    for i in range(len(state['corridors']), len(stations)-1):
        ...
        if 'active' not in state:
            ...
            if i == 1:
                # Reviewed 26 m pair on the same -2 layer; retain the later
                # control before the return to -1, without changing source data.
                points = [p for p in points if p.get('osm_id') != 11934983991]
        ...
        del state['active']
        save()
        print(f'Corridor {i+1}/3 connected', flush=True)
