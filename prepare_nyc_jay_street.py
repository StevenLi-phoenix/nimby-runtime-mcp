"""Plan Jay Street's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    ...
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][24]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [904597215, 904597216]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 818608013)
    ...
    for w in source['ways']:
        if w['tags'].get('railway') != 'subway' or w['tags'].get('level') != '-2':
            continue
        if w['tags'].get('name') not in ['IND Culver Line', 'IND Fulton Street Line']:
            continue
    ...
    return dict(name='Jay Street–MetroTech', plan_index=24, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_jay_street_source.json',
                reserved_broadway_platform=dict(source_way_id=904597698,depth=-3),
                stage='candidate_native_platform_audit_pending',
                pending=['Unify IND platforms under one native station.',
                         'Connect F south to Bergen and north to York Street.',
                         'Preserve A/C and R station geometry for full network construction.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_jay_street_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
