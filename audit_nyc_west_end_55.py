...
p=json.loads(Path('data/nyc_west_end_55_side_platforms.json').read_bytes());v=json.loads(Path('work/nyc-west_end_55-native.json').read_bytes());c=p['center'];tx,ty=p['southbound_axis'].values();scale=p['scale'];fig,ax=plt.subplots(figsize=(6,9));widths=json.loads(Path('data/nyc_west_end_55_platform_widths.json').read_bytes());faces=0
...
ax.set(aspect='equal',title='55th Street side platforms');ax.grid(alpha=.2);fig.tight_layout();fig.savefig(Path(os.environ['TEMP'])/'nyc-west_end_55-audit.png',dpi=130)
