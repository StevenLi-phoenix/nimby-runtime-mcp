"""Validate two islands surrounding one shared central platform track at Kings Highway."""
import json
import math
import os
...
def audit():
    ...
    s=next(s for s in f['stations'] if s['plan_index']==39)
    src=json.loads(Path('data/nyc_mid_culver_source.json').read_bytes())['stations'][3]
    net=json.loads(Path('work/nyc-kings-highway-native.json').read_bytes());nodes={n['id']:n for n in net['nodes']}
    ...
    assert len(faces)==16
    central=set(f['kings_highway_center_platform']['platform_nodes'])
    for i in central:
        assert {b['island'] for b in faces if b['node_id']==i and b['type']==13}=={0,1}
    ax.set(aspect='equal',ylim=(125,-125),xlim=(-14,14),title='Kings Highway: two islands, shared center rail',xlabel='Metres across',ylabel='Metres south')
    ax.grid(alpha=.2);fig.tight_layout();fig.savefig(Path(os.environ['TEMP'])/'nyc-kings-highway-audit.png',dpi=160)
    north=[s['west_primary'],s['west_secondary'],f['kings_highway_center_platform']['north_endpoint']]
    ...
    result=dict(station_id=s['native_station'],nodes=12,buildings=22,physical_tracks=3,platform_lengths_metres=lengths,
                ...
                faces=faces,construction_pending=any(n['blueprint'] for n in nodes.values()),north_connection_pending=not connected)
    Path('work/nyc-kings-highway-audit.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:v for k,v in result.items() if k!='faces'}))
