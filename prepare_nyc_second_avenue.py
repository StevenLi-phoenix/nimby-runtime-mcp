"""Record 2nd Avenue's full station cross-section, including deferred yard tracks."""
import json
import math
...
def prepare():
    ...
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][20]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [907929300,907929301]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 818168009)
    ...
    for w in source['ways']:
        if w['tags'].get('railway') != 'subway' or w['tags'].get('level') != '-2':
            continue
        if w['tags'].get('name') != 'IND Sixth Avenue Line' and w['osm_way_id'] not in [818168004,818168005]:
            continue
        for a, b in zip(w['points'], w['points'][1:]):
            ...
            if abs(offset(q)) < 40:
                cross.append(dict(source_way_id=w['osm_way_id'], track_ref=w['tags']['railway:track_ref'],
                                  line_name=w['tags'].get('name',''), service=w['tags'].get('service'), offset_metres=offset(q), **q))
    ...
    return dict(name='2nd Avenue', plan_index=20, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_delancey_source.json',
                deferred_yard_track_refs=['5','6'],
                stage='source_layout_only_central_yard_tracks_deferred',
                pending=['Plan B1/B2 running tracks and platform faces without building B5/B6 yard tracks.',
                         'Connect F south to Delancey and north to Broadway–Lafayette.',
                         'Retain deferred B5/B6 source geometry without creating yard extensions.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_second_avenue_source_layout.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
