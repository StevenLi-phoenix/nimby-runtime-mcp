...
def prepare():
    survey = json.loads(Path('data/nyc_36_ninth_survey.json').read_bytes())
    nodes = {n['id']: n for n in json.loads(Path('work/nyc-36-ninth-live.json').read_bytes())['nodes']}
    corridors = []
    for source in survey['corridors']:
        route = source['source_route']
        end = source['length_metres']
        profile = [(0,-2),(25,-2),(90,-1),(170,-1),(end,-1)]
    ...
    return dict(scale=survey['scale'], corridors=corridors,
                source='data/nyc_36_ninth_survey.json',
                stage='candidate_native_geometry_and_crossing_audit_pending',
                vertical_inference='Missing-tag western segments use -1 continuously, inferred from adjacent tagged cut and station tracks; open cut is represented below street level.',
...
if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_36_ninth_corridors.json').write_text(json.dumps(result,indent=2)+'\n',encoding='utf-8')
