"""Create Yizhuang Line station blueprints only; never commit construction implicitly."""
import hashlib
import json
import math
...
CHECKPOINT = Path('work/yizhuang-built.json')


def run():
    raw = Path('data/yizhuang_plan.json').read_bytes()
    plan = json.loads(raw)
