"""Plan the Sixth Avenue connection express tracks from Herald Square to West 4th Street."""
import json
...
def prepare():
    ...
    source = json.loads(Path('data/nyc_sixth_express_source.json').read_bytes())
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-sixth-express-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_herald_islands.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('3', 0, 'east_primary'), ('4', 1, 'east_secondary')]:
        a = state['herald_islands'][index][key]
        b = state['west_fourth_islands'][index][key.replace('east','west')]
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_sixth_express_source.json',
                stage='candidate_native_curves_and_crossing_audit_pending',
                adaptation='Source -4 compressed to -3; Canarsie crossing needs explicit adaptation; original depths remain in source_layer_segments.',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_sixth_express_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
