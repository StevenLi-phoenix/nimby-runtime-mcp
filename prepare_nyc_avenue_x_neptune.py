"""Plan three source-connected elevated tracks from Avenue X to Neptune Avenue."""
import json
...
def prepare():
    ...
    nodes={n['id']:n for name in ['nyc-avenue-x-neptune-live'] for n in json.loads(Path('work/'+name+'.json').read_bytes())['nodes']}
    s={s['plan_index']:s for s in f['stations']}
    ends={'1':(s[41]['east_secondary'],s[42]['west_secondary']),
          '2':(s[41]['east_primary'],s[42]['west_primary'])}
    scale=json.loads(Path('data/nyc_neptune_island.json').read_bytes())['scale'];corridors=[]
...
if __name__=='__main__':
    result=prepare();Path('data/nyc_avenue_x_neptune_corridors.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
