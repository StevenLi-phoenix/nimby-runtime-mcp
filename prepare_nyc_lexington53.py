"""Plan Lexington Avenue-53rd Street's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    ...
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][11]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [907542974]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 1546629369)
    ...
    return dict(name='Lexington Avenue-53rd Street', plan_index=11, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-3, platform_length_metres=140, source='data/nyc_lexington53_source.json',
                stage='candidate_native_platform_audit_pending',
                pending=['Unify IND platforms under one native station.',
                         'Connect Fifth Avenue to the west and the East River tunnel to the east.',
                         'Preserve Lexington Avenue interchange tracks above this station.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_lexington53_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
