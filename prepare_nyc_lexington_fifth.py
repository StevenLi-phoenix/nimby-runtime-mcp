"""Plan the Sixth Avenue connection from Lexington Avenue to Fifth Avenue on 53rd Street."""
import json
...
def prepare():
    ...
    source = json.loads(Path('data/nyc_lexington53_source.json').read_bytes())
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-lexington-fifth-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_lexington53_islands.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('3',0,'east_secondary'),('4',1,'east_primary')]:
        a = state['lexington53_islands'][0][key]
        b = state['fifth_53rd_platforms'][index]['east_lead']
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_lexington53_source.json',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_lexington_fifth_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
