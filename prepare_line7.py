"""Validate Line 7 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '北京西站 湾子 达官营 广安门内 菜市口 虎坊桥 珠市口 桥湾 磁器口 广渠门内 广渠门外 双井 九龙山 大郊亭 百子湾 化工 南楼梓庄 欢乐谷景区 垡头 双合 焦化厂 黄厂 郎辛庄 黑庄户 万盛西 万盛东 群芳 高楼金 花庄 环球度假区'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 4623397]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 7 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/4623397/full.json',
                ...
                scope='北京地铁7号线北京西站—环球度假区，完整30站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-7.html',
                code='bj-7', color='#FFC66E',
...
if __name__ == '__main__':
    ...
    Path('data/line7_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
