"""Plan Grand Street Queens Boulevard side platforms."""
import json
import math
...
def prepare():
    source=json.loads(Path('data/nyc_grand_source.json').read_bytes())
    stop=json.loads(Path('data/nyc_d_plan.json').read_bytes())['directions']['to_coney_island']['stations'][20]
    footprints=[w for w in source['ways'] if w['osm_way_id'] in [907932319,907932320]]
    ...
    rails=[w for w in source['ways'] if w['tags'].get('name')=='Chrystie Street Connection' and w['tags'].get('level')=='-2']
    ...
    assert 3<spacing<30
    return dict(name='Grand Street',plan_index=20,center=center,scale=scale,
                ...
                source_platforms=footprints,reserved_express_track_refs=[],
                reserved_express_depth=-2,source='data/nyc_grand_source.json',
                stage='candidate_native_creation_pending')


if __name__=='__main__':
    p=prepare();Path('data/nyc_grand_side_platforms.json').write_text(json.dumps(p,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
