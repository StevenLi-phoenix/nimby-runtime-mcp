"""Validate Line 12 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '四季青桥 蓝靛厂 长春桥 苏州桥 人民大学 大钟寺 蓟门桥 北太平庄 马甸桥 安华桥 安贞桥 和平西桥 光熙门 西坝河 三元桥 将台西 高家园 驼房营 东坝西 东坝北'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 18441519]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 12 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/18441519/full.json',
                ...
                scope='北京地铁12号线四季青桥—东坝北，当前完整20座运营站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-12.html',
                code='bj-12', color='#9C4F24',
...
if __name__ == '__main__':
    ...
    Path('data/line12_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
