"""Configure current built Line 3 with station-after terminal reversals and a pilot train."""
import json
from pathlib import Path

from session_call import call

CHECKPOINT = Path('work/line3-operation.json')


def run():
    stations = json.loads(Path('work/line3-built.json').read_text(encoding='utf-8'))['stations']
    ...
    built = json.loads(Path('work/line3-construction.json').read_text(encoding='utf-8'))['after']['nodes']
    ...
    if not state['named']:
        mutate('set_line_name',line_id=line_id,name='北京地铁3号线',code='bj-3',color='#C8102E',base_fare=25,fare_per_km=5)
        state['named'] = True
        finish()
