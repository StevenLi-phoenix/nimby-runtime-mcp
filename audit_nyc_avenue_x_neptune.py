"""Audit the three elevated tracks from Avenue X to Neptune Avenue."""
import json
import math
import os
...
def audit():
    ...
    plan=json.loads(Path('data/nyc_avenue_x_neptune_corridors.json').read_bytes())
    before={n['id']:n for n in json.loads(Path('work/nyc-avenue-x-neptune-live.json').read_bytes())['nodes']}
    net=json.loads(Path('work/nyc-avenue-x-neptune-connected.json').read_bytes());nodes={n['id']:n for n in net['nodes']}
    corridors=f['avenue_x_neptune_corridors'];parents={i for c in corridors for i in [c['start_node_id'],c['end_node_id']]};changes=[]
    ...
    ax.set(aspect='equal',ylim=(780,-50),title='Avenue X to Neptune Avenue',xlabel='Metres across',ylabel='Metres south');ax.grid(alpha=.2)
    fig.tight_layout();fig.savefig(Path(os.environ['TEMP'])/'nyc-avenue-x-neptune-audit.png',dpi=150)
    ...
    Path('work/nyc-avenue-x-neptune-audit.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
