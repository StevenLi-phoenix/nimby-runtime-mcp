...
s=json.loads(Path('data/nyc_west_end_79_source.json').read_bytes());stop=json.loads(Path('data/nyc_d_plan.json').read_bytes())['directions']['to_coney_island']['stations'][29];centers=[]
for w in s['ways']:
 if w['osm_way_id'] in [427044445,427044446]:
...
p=dict(name='79th Street',plan_index=29,center=c,scale=scale,southbound_axis=dict(x=tx,y=ty),tracks=tracks,source_platform_way_ids=[427044445,427044446],source='data/nyc_west_end_79_source.json',layout='two_side_platforms_with_center_express_track',depth=2,stage='candidate_native_creation_pending');Path('data/nyc_west_end_79_three_tracks.json').write_text(json.dumps(p,indent=2));print(json.dumps(rows))
