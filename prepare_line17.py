"""Validate Line 17 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '未来科学城北 未来科学城 天通苑东 清河营 红军营 望京西 太阳宫 西坝河 左家庄 工人体育场 东大桥 永安里 潘家园西 十里河 周家庄 十八里店 北神树 次渠北 次渠 嘉会湖'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 13625142]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 17 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/13625142/full.json',
                ...
                scope='北京地铁17号线未来科学城北—嘉会湖，当前完整20座运营站；广渠门外暂缓',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-17.html',
                code='bj-17', color='#00A9A9',
...
if __name__ == '__main__':
    ...
    Path('data/line17_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
