"""Configure current built Yizhuang T1 with station-after terminal reversals and a pilot train."""
import json
from pathlib import Path

from session_call import call

CHECKPOINT = Path('work/yizhuang_t1-operation.json')
...
def run():
    ...
    built = json.loads(Path('work/yizhuang_t1-construction.json').read_text(encoding='utf-8'))['after']['nodes']
    ...
    if not state['named']:
        mutate('set_line_name',line_id=line_id,name='北京亦庄T1线',code='bj-t1',color='#D22630',base_fare=25,fare_per_km=5)
        state['named'] = True
        finish()
