"""Audit the three elevated tracks from Avenue N to Avenue P."""
import json
import math
import os
...
def audit():
    ...
    plan=json.loads(Path('data/nyc_avenue_n_avenue_p_corridors.json').read_bytes())
    before={n['id']:n for n in json.loads(Path('work/nyc-avenue-n-avenue-p-live.json').read_bytes())['nodes']}
    net=json.loads(Path('work/nyc-avenue-n-avenue-p-connected.json').read_bytes());nodes={n['id']:n for n in net['nodes']}
    corridors=f['avenue_n_avenue_p_corridors'];parents={i for c in corridors for i in [c['start_node_id'],c['end_node_id']]};changes=[]
    ...
    ax.set(aspect='equal',ylim=(500,-50),title='Avenue N to Avenue P',xlabel='Metres across',ylabel='Metres south');ax.grid(alpha=.2)
    fig.tight_layout();fig.savefig(Path(os.environ['TEMP'])/'nyc-avenue-n-avenue-p-audit.png',dpi=150)
    ...
    Path('work/nyc-avenue-n-avenue-p-audit.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
