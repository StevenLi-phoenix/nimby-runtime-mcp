"""Plan three source-connected elevated tracks from Bay Parkway to Avenue N."""
import json
...
def prepare():
    ...
    nodes={n['id']:n for name in ['nyc-bay-parkway-avenue-n-live'] for n in json.loads(Path('work/'+name+'.json').read_bytes())['nodes']}
    s={s['plan_index']:s for s in f['stations']}
    ends={'1':(s[36]['east_secondary'],s[37]['west_secondary']),
          '2':(s[36]['east_primary'],s[37]['west_primary']),
          '3-4':(f['bay_parkway_center_track']['south_endpoint'],f['avenue_n_center_track']['north_endpoint'])}
    scale=next(s for s in source['stations'] if s['plan_index']==37)['scale'];corridors=[]
...
if __name__=='__main__':
    result=prepare();Path('data/nyc_bay_parkway_avenue_n_corridors.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
