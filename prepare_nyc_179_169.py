"""Plan four through tracks between Jamaica-179th Street and 169th Street; yard links excluded."""
import json
...
def prepare():
    ...
    ns = {n['id']: n for n in json.loads(Path('work/nyc-179-169-live.json').read_bytes())['nodes']}
    source_path = 'data/nyc_179_169_source.json'
    source = json.loads(Path(source_path).read_bytes())
    scale = json.loads(Path('data/nyc_179_islands.json').read_bytes())['scale']
    local=s['169_platform_groups'][0]
    express={e['track_ref']:e for e in s['169_express_tracks']}
    pairs=[('1',s['179_islands'][0]['east_secondary'],local['west_secondary']),
           ('3',s['179_islands'][0]['east_primary'],express['3']['west']),
           ('4',s['179_islands'][1]['east_secondary'],express['4']['west']),
           ('2',s['179_islands'][1]['east_primary'],local['west_primary'])]
...
if __name__ == '__main__':
    p = prepare()
    Path('data/nyc_179_169_corridors.json').write_text(json.dumps(p, indent=2)+'\n')
