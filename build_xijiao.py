"""Create Xijiao Line station blueprints only; never commit construction implicitly."""
import hashlib
import json
import math
...
CHECKPOINT = Path('work/xijiao-built.json')


def run():
    raw = Path('data/xijiao_plan.json').read_bytes()
    plan = json.loads(raw)
    ...
    for index, s in enumerate(plan['stations']):
        ...
        if record['stage'] == 'assigned':
            ...
            save()
        print(f"Station {index+1}/6 verified", flush=True)
