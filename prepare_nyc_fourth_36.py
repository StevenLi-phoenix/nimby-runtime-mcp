"""Plan 36th Street's BMT four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_fourth_36_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_d_plan.json').read_bytes())['directions']['to_coney_island']['stations'][22]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [907182639,907182640]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 817936183)
    ...
    for w in source['ways']:
        if w['tags'].get('railway') != 'subway' or w['tags'].get('level') != '-2':
            continue
        if w['tags'].get('name') not in ['BMT Fourth Avenue Line']:
            continue
    ...
    return dict(name='36th Street', plan_index=22, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_fourth_36_source.json',
                stage='candidate_native_platform_audit_pending',
                pending=['Unify Fourth Avenue platforms under one native station.',
                         'Connect Fourth Avenue express and local tracks; preserve the West End branch.',
                         'Preserve nearby yard access as deferred work.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_fourth_36_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
