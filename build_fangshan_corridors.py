"""Connect current Fangshan Line station blueprints; never build or restore old station geometry."""
import bisect
import json
import math
import sys
...
CHECKPOINT = Path('work/fangshan-corridors.json')


def run():
    stations = json.loads(Path('work/fangshan-built.json').read_text(encoding='utf-8'))['stations']
    plan = json.loads(Path('data/fangshan_plan.json').read_text(encoding='utf-8'))
    ...
    for i in range(len(state['corridors']), len(stations)-1):
        ...
        del state['active']
        save()
        print(f'Corridor {i+1}/15 connected', flush=True)
