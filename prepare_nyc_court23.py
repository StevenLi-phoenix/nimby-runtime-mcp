...
def prepare():
    ...
    stop=json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][10]
    footprints=[w for w in source['ways'] if w['osm_way_id'] in [721007405,818637922]]
    ...
    assert 3<spacing<30
    return dict(name='Court Square-23rd Street',plan_index=10,center=center,scale=scale,
                ...
                source_platforms=footprints,existing_interchange_station_id="562949983109121",
                reserved_path_depth=-2,stage='candidate_native_creation_pending')


if __name__=='__main__':
    p=prepare();Path('data/nyc_court23_side_platforms.json').write_text(json.dumps(p,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
