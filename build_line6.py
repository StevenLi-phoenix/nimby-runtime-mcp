"""Create Line 6 station blueprints only; never commit construction implicitly."""
import hashlib
import json
import math
...
CHECKPOINT = Path('work/line6-built.json')


def run():
    raw = Path('data/line6_plan.json').read_bytes()
    plan = json.loads(raw)
    ...
    for index, s in enumerate(plan['stations']):
        ...
        if record['stage'] == 'assigned':
            ...
            save()
        print(f"Station {index+1}/36 verified", flush=True)
