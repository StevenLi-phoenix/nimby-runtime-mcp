"""Plan three source-connected elevated tracks from Avenue P to Kings Highway."""
import json
...
def prepare():
    ...
    source=json.loads(Path('data/nyc_mid_culver_source.json').read_bytes())
    nodes={n['id']:n for name in ['nyc-avenue-p-kings-highway-live'] for n in json.loads(Path('work/'+name+'.json').read_bytes())['nodes']}
    s={s['plan_index']:s for s in f['stations']}
    ends={'1':(s[38]['east_secondary'],s[39]['west_secondary']),
          '2':(s[38]['east_primary'],s[39]['west_primary']),
          '3-4':(f['avenue_p_center_track']['south_endpoint'],f['kings_highway_center_platform']['north_endpoint'])}
    scale=next(s for s in source['stations'] if s['plan_index']==39)['scale'];corridors=[]
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_mid_culver_source.json',
                stage='candidate_native_audit_pending',attribution='© OpenStreetMap contributors; https://www.openstreetmap.org/copyright')


if __name__=='__main__':
    result=prepare();Path('data/nyc_avenue_p_kings_highway_corridors.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
