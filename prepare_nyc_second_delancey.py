"""Plan the Sixth Avenue connection from 2nd Avenue to Delancey Street."""
import json
...
def prepare():
    ...
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-second-delancey-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_second_avenue_running_platforms.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('1', 0, 'east_secondary'), ('2', 1, 'east_primary')]:
        a = state['second_avenue_platform_groups'][0][key]
        b = state['delancey_platform_groups'][0]['west_secondary' if ref == '1' else 'west_primary']
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_delancey_source.json',
                stage='candidate_native_curves_and_crossing_audit_pending',
                adaptation='Source -2 to -3 transition retained; original depths remain in source_layer_segments.',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_second_delancey_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
