"""Plan Jackson Heights-Roosevelt Avenue's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_jackson_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][8]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [515886157,515886158]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 813738323)
    ...
    return dict(name='Jackson Heights-Roosevelt Avenue', plan_index=8, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-3, platform_length_metres=140, source='data/nyc_jackson_source.json',
                ...
                pending=['Unify IND platforms under one native station.',
                         ...
                         'Preserve the elevated Flushing interchange source.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_jackson_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
