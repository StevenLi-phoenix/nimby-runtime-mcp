"""Create Daxing Airport Line station blueprints only; never commit construction implicitly."""
import hashlib
import json
import math
...
CHECKPOINT = Path('work/daxing_airport-built.json')


def run():
    raw = Path('data/daxing_airport_plan.json').read_bytes()
    plan = json.loads(raw)
    ...
    for index, s in enumerate(plan['stations']):
        ...
        if record['stage'] == 'assigned':
            ...
            save()
        print(f"Station {index+1}/3 verified", flush=True)
