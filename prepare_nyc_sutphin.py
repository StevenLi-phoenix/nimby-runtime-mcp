"""Plan Sutphin Boulevard Queens Boulevard side platforms."""
import json
import math
...
def prepare():
    source=json.loads(Path('data/nyc_sutphin_source.json').read_bytes())
    stop=json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][3]
    footprints=[w for w in source['ways'] if w['osm_way_id'] in [906713687,906713688]]
    ...
    assert 3<spacing<30
    return dict(name='Sutphin Boulevard',plan_index=3,center=center,scale=scale,
                ...
                reserved_express_depth=-2,source='data/nyc_sutphin_source.json',
                stage='candidate_native_creation_pending')


if __name__=='__main__':
    p=prepare();Path('data/nyc_sutphin_side_platforms.json').write_text(json.dumps(p,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
