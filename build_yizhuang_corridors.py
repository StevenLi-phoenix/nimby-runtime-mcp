"""Connect current Yizhuang Line station blueprints; never build or restore old station geometry."""
import bisect
import json
import math
import sys
...
CHECKPOINT = Path('work/yizhuang-corridors.json')


def run():
    stations = json.loads(Path('work/yizhuang-built.json').read_text(encoding='utf-8'))['stations']
    plan = json.loads(Path('data/yizhuang_plan.json').read_text(encoding='utf-8'))
