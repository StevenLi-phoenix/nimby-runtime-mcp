"""Create Yizhuang T1 station blueprints only; never commit construction implicitly."""
import hashlib
import json
import math
...
CHECKPOINT = Path('work/yizhuang_t1-built.json')


def run():
    raw = Path('data/yizhuang_t1_plan.json').read_bytes()
    plan = json.loads(raw)
    ...
    for index, s in enumerate(plan['stations']):
        ...
        if record['stage'] == 'assigned':
            ...
            save()
        print(f"Station {index+1}/14 verified", flush=True)
