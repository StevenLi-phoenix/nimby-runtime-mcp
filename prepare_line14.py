"""Validate Line 14 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '张郭庄 园博园 大瓦窑 郭庄子 大井 七里庄 西局 东管头 丽泽商务区 菜户营 西铁营 景风门 北京南站 永定门外 景泰 蒲黄榆 方庄 十里河 北工大西门 平乐园 九龙山 大望路 金台路 朝阳公园 枣营 东风北桥 将台 望京南 阜通 望京 东湖渠 来广营 善各庄'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 4613036]
    ...
    # Hongmiao is included prematurely in OSM route stops; the current
    # operator timetable excludes it and official plans target late 2026.
    stops = [n for n in stops if n['tags']['name'] != '红庙']
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 14 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/4613036/full.json',
                ...
                scope='北京地铁14号线张郭庄—善各庄，完整33座运营站',
                excluded_scope=['车辆段', '联络线', '库线', '尚未开放的红庙/陶然桥/高家园运营停站'],
                official_scope='https://www.mtr.bj.cn/service/line/timetable/line-14.html',
                code='bj-14', color='#D4A7A1',
...
if __name__ == '__main__':
    ...
    Path('data/line14_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
