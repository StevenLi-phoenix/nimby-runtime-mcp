"""Plan 14th Street Sixth Avenue side platforms."""
import json
import math
...
def prepare():
    ...
    stop=json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][17]
    footprints=[w for w in source['ways'] if w['osm_way_id'] in [554392754,554392756]]
    ...
    rails=[w for w in source['ways'] if w['tags'].get('name')=='IND Sixth Avenue Line' and w['tags'].get('level')=='-2']
    ...
    assert 3<spacing<30
    return dict(name='14th Street | Sixth Avenue',plan_index=17,center=center,scale=scale,
                ...
                source_platforms=footprints,reserved_canarsie_platforms=[552734862],
                reserved_canarsie_depth=-3,stage='candidate_native_creation_pending')


if __name__=='__main__':
    p=prepare();Path('data/nyc_fourteenth_side_platforms.json').write_text(json.dumps(p,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
