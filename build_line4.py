"""Create Line 4 station blueprints only; never commit construction implicitly."""
import hashlib
import json
import math
...
CHECKPOINT = Path('work/line4-built.json')


def run():
    raw = Path('data/line4_plan.json').read_bytes()
    plan = json.loads(raw)
    ...
    for index, s in enumerate(plan['stations']):
        ...
        if record['stage'] == 'assigned':
            ...
            save()
        print(f"Station {index+1}/24 verified", flush=True)
    state['status'] = 'station-blueprints-awaiting-corridors-and-footprint-review'
    save()
