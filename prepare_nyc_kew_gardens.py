"""Plan Kew Gardens-Union Turnpike's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_kew_gardens_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][5]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [906721628,906721629]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 818901100)
    ...
    return dict(name='Kew Gardens-Union Turnpike', plan_index=5, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_kew_gardens_source.json',
...
if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_kew_gardens_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
