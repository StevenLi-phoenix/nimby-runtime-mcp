"""Plan the Sixth Avenue connection from Broadway-Lafayette Street to 2nd Avenue."""
import json
...
def prepare():
    ...
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-broadway-second-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_broadway_lafayette_islands.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('1', 0, 'east_secondary'), ('2', 1, 'east_primary')]:
        a = state['broadway_lafayette_islands'][index][key]
        b = state['second_avenue_platform_groups'][0]['west_secondary' if ref == '1' else 'west_primary']
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_broadway_lafayette_source.json',
                stage='candidate_native_curves_and_crossing_audit_pending',
                adaptation='Source -3 to -2 transition retained; original depths remain in source_layer_segments.',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_broadway_second_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
