"""Validate Line 15 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '清华东路西口 六道口 北沙滩 奥林匹克公园 安立路 大屯路东 关庄 望京西 望京 望京东 崔各庄 马泉营 孙河 国展 花梨坎 后沙峪 南法信 石门 顺义 俸伯'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 1350597]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 15 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/1350597/full.json',
                ...
                scope='北京地铁15号线清华东路西口—俸伯，完整20站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.bjsubway.com/station/xltcx/line15/2013-10-26/358.html',
                code='bj-15', color='#653279',
...
if __name__ == '__main__':
    ...
    Path('data/line15_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
