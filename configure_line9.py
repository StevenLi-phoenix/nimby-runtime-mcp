"""Configure current built Line 9 with station-after terminal reversals and a pilot train."""
import json
from pathlib import Path

from session_call import call

CHECKPOINT = Path('work/line9-operation.json')
...
def run():
    ...
    built = json.loads(Path('work/line9-construction.json').read_text(encoding='utf-8'))['after']['nodes']
    ...
    if not state['named']:
        mutate('set_line_name',line_id=line_id,name='北京地铁9号线',code='bj-9',color='#87D300',base_fare=25,fare_per_km=5)
        state['named'] = True
        finish()
    ...
    for index in range(line['stop_count'],len(stops)):
        ...
        finish()
        print(f'Stop {index+1}/26 configured',flush=True)
    ...
    save()
    print(json.dumps(dict(line_id=line_id,train_id=state['fleet'][0]['id'],stops=26)))
