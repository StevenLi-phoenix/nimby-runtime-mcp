"""Connect York Street platforms to Jay Street."""
import json
...
def prepare():
    ...
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-york-jay-live.json').read_bytes())['nodes']}
    ...
    for ref, index, key in [('1', 0, 'east_secondary'), ('2', 1, 'east_primary')]:
        a = state['york_islands'][0][key]
        b = state['jay_islands'][index]['west_secondary' if ref == '1' else 'west_primary']
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_york_street_source.json',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_york_jay_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
