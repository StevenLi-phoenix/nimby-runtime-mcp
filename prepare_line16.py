"""Validate Line 16 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '北安河 温阳路 稻香湖路 屯佃 永丰 永丰南 西北旺 马连洼 农大南路 西苑 万泉河桥 苏州街 苏州桥 万寿寺 国家图书馆 二里沟 甘家口 玉渊潭东门 木樨地 达官营 红莲南路 丽泽商务区 东管头南 丰台站 丰台南路 富丰桥 看丹 榆树庄 洪泰庄 宛平城'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 7800400]
    ...
    if survey_path:
        ...
        for station in stations:
            ...
            if current['matches']:
                ...
                occupied = {n['depth'] for n in current['nearby']['nodes'] + current.get('matched_station_nearby', {'nodes': []})['nodes']
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/7800400/full.json',
                ...
                scope='北京地铁16号线北安河—宛平城，完整30站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/timetable/line-16.html',
                code='bj-16', color='#6DA843',
...
if __name__ == '__main__':
    ...
    Path('data/line16_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
