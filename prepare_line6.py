"""Validate Line 6 geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
EXPECTED = '金安桥 苹果园 杨庄 西黄村 廖公庄 田村 海淀五路居 慈寿寺 花园桥 白石桥南 二里沟 车公庄西 车公庄 平安里 北海北 南锣鼓巷 东四 朝阳门 东大桥 呼家楼 金台路 十里堡 青年路 褡裢坡 黄渠 常营 草房 物资学院路 通州北关 通运门 北运河西 北运河东 郝家府 东夏园 潞城 潞阳'.split()
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 4625141]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Line 6 station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/4625141/full.json',
                ...
                scope='北京地铁6号线金安桥—潞阳，完整36站，含通运门',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.bjsubway.com/station/xltcx/line6/2025-12-28/653.html',
                code='bj-6', color='#BB8900',
...
if __name__ == '__main__':
    ...
    Path('data/line6_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
