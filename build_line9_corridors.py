"""Connect current Line 9 station blueprints; never build or restore old station geometry."""
import bisect
import json
import math
import sys
...
CHECKPOINT = Path('work/line9-corridors.json')


def run():
    stations = json.loads(Path('work/line9-built.json').read_text(encoding='utf-8'))['stations']
    plan = json.loads(Path('data/line9_plan.json').read_text(encoding='utf-8'))
    ...
    for i in range(len(state['corridors']), len(stations)-1):
        ...
        if 'active' not in state:
            ...
            if i == 6:
                # Keep the depth boundary, omit a same-depth interior control
                # immediately after it; retain both live station endpoints.
                j = 1
                while j < len(points)-2:
                    if points[j]['depth'] == points[j+1]['depth'] and distance(points[j],points[j+1]) < 35:
                        del points[j+1]
                    else:
                        j += 1
            if i == 9:
                # Keep both depth-boundary controls; omit the redundant point
                # just before the boundary rather than a 31 m grade segment.
                points = [p for p in points if p.get('osm_id') != 12373498074]
        ...
        del state['active']
        save()
        print(f'Corridor {i+1}/12 connected', flush=True)
