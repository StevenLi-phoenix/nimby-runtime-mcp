"""Plan Parsons Boulevard's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    source = json.loads(Path('data/nyc_parsons_source.json').read_bytes())
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][2]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [906711097,906711098]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 818901096)
    ...
    return dict(name='Parsons Boulevard', plan_index=2, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_parsons_source.json',
...
if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_parsons_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
