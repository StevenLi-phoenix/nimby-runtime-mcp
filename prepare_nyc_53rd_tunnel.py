"""Plan the Sixth Avenue connection from Court Square to Lexington Avenue through the 53rd Street tunnel."""
import json
...
def prepare():
    ...
    source = json.loads(Path('data/nyc_53rd_tunnel_source.json').read_bytes())
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-restart-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_court23_side_platforms.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('3',0,'east_secondary'),('4',1,'east_primary')]:
        a = state['court23_platform_groups'][0][key]
        b = state['lexington53_islands'][0][key.replace('east','west')]
        assert all('0' in [nodes[i]['previous'],nodes[i]['next']] for i in [a,b])
        eligible = [dict(w,tags=dict(w['tags'],name='IND Culver Line')) for w in source['ways'] if w['tags'].get('name')=='IND Queens Boulevard Line']
        route, ways, gaps = route_between(eligible,ref,nodes[a],nodes[b])
        ...
        # Spread the native descent over the straight approach rather than
        # concentrating a full level change into a 27 m source curve segment.
        transition = next(chain[i] for i in range(1,len(depths)) if depths[i-1] == -2 and depths[i] == -3)
        descent_start = transition - 180
        for point in points:
            if transition - 90 <= point['metres'] < transition:
                point['depth'] = -3
        i = next(i for i in range(len(chain)-1) if chain[i] <= descent_start <= chain[i+1])
        f = (descent_start-chain[i])/(chain[i+1]-chain[i])
        points.append(dict(x=route[i]['x']+f*(route[i+1]['x']-route[i]['x']),
                           y=route[i]['y']+f*(route[i+1]['y']-route[i]['y']),
                           metres=descent_start,depth=-2))
        points.sort(key=lambda point:point['metres'])
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_53rd_tunnel_source.json',
                stage='candidate_native_curves_and_crossing_audit_pending',
                adaptation='Source stacked-track transition retained; original depths remain in source_layer_segments.',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_53rd_tunnel_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
