"""Plan the Sixth Avenue connection from 14th Street to West 4th Street."""
import json
...
def prepare():
    ...
    source = json.loads(Path('data/nyc_fourteenth_source.json').read_bytes())
    nodes = {n['id']:n for n in json.loads(Path('work/nyc-fourteenth-west-live.json').read_bytes())['nodes']}
    scale = json.loads(Path('data/nyc_fourteenth_side_platforms.json').read_bytes())['scale']
    corridors = []
    for ref, index, key in [('1', 0, 'east_secondary'), ('2', 1, 'east_primary')]:
        a = state['fourteenth_platform_groups'][0][key]
        b = state['west_fourth_islands'][index]['west_secondary' if ref == '1' else 'west_primary']
    ...
    return dict(corridors=corridors,scale=scale,source='data/nyc_fourteenth_source.json',
                stage='candidate_native_curves_and_crossing_audit_pending',
                adaptation='Source -4 maps to native -3; other source levels retained; original depths remain in source_layer_segments.',
...
if __name__=='__main__':
    result=prepare()
    Path('data/nyc_fourteenth_west_corridors.json').write_text(json.dumps(result,indent=2)+'\n')
