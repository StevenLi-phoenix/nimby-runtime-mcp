"""Connect current Yanfang Line station blueprints; never build or restore old station geometry."""
import bisect
import json
import math
import sys
...
CHECKPOINT = Path('work/yanfang-corridors.json')


def run():
    stations = json.loads(Path('work/yanfang-built.json').read_text(encoding='utf-8'))['stations']
    plan = json.loads(Path('data/yanfang_plan.json').read_text(encoding='utf-8'))
    ...
    for i in range(len(state['corridors']), len(stations)-1):
        ...
        del state['active']
        save()
        print(f'Corridor {i+1}/8 connected', flush=True)
