"""Validate Daxing Airport Line geometry and preserve source bridge/tunnel boundaries."""
import argparse
import bisect
import hashlib
import json
...
def prepare(path, survey_path=None):
    ...
    relation = objects['relation', 10136949]
    ...
    if [normalize(n['tags']['name']) for n in stops] != EXPECTED:
        raise ValueError('Unexpected Daxing Airport Line station order')
    ...
    return dict(source='https://api.openstreetmap.org/api/0.6/relation/10136949/full.json',
                ...
                scope='北京大兴机场线草桥—大兴机场，当前完整3座运营站',
                excluded_scope=['车辆段', '联络线', '库线'],
                official_scope='https://www.mtr.bj.cn/service/line/distable/line-daxingjichang.html',
                code='bj-dae', color='#3F48CC',
...
if __name__ == '__main__':
    ...
    Path('data/daxing_airport_plan.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
