"""Plan Queens Plaza's IND four-track, two-island station from source geometry."""
import json
import math
...
def prepare():
    ...
    stop = json.loads(Path('data/nyc_f_plan.json').read_bytes())['directions']['to_coney_island']['stations'][9]
    footprints = [w for w in source['ways'] if w['osm_way_id'] in [906957491,906957492]]
    ...
    ref = next(w for w in source['ways'] if w['osm_way_id'] == 818637878)
    ...
    return dict(name='Queens Plaza', plan_index=9, center=center, scale=scale,
                southbound_axis=dict(x=tx,y=ty), cross_section=cross, islands=islands,
                depth=-2, platform_length_metres=140, source='data/nyc_court23_source.json',
                stage='candidate_native_platform_audit_pending',
                pending=['Unify IND platforms under one native station.',
                         'Connect the Queens Boulevard four-track corridor and the Court Square branch.',
                         'Verify junctions and retain the separate Queensboro Plaza source geometry.'])


if __name__ == '__main__':
    result = prepare()
    Path('data/nyc_queens_plaza_islands.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
